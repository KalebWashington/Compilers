...
/* Kian Pokorny January 8, 2010
This file contains an interpreter for the x machine. 
The input file must be in the same directory as the 
interpreter.  At the prompt the file name is typed in 
including any extension.

Fix all code functions that have 
		...
		while (!done) {
			...
			*/
			...
			case TNNF : /* Test Not Negative  (floating-point)*/
				if (real(pop()) >= 0) push(0xFFFFFFFF); else push((unsigned int)0);
				break;
